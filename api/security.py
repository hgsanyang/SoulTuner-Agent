...
def admin_key_required() -> bool:
    """Whether the destructive ops (delete / settings / rebuild) need the key.

    True once you set ADMIN_API_KEY, or in shared safe mode. Everything else —
    recommend, library, feedback — stays open: the browser sends no key, so
    gating those would just lock you out of your own app.
    """
    return bool(_admin_api_key()) or shared_safe_mode_enabled()
