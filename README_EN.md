...
stateDiagram-v2
    ...
    web_fallback --> explain_results: Netease API live search results
...
> `web_search` intent now routes **directly to the `web_fallback` node** (Netease API live search), bypassing HybridRetrieval entirely. Supports Chinese-first query extraction, multi-level fallback query resolution, and 30-second preview detection.
