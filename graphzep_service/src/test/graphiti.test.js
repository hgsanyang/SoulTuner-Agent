...
import { GraphProvider, EpisodeType } from '../types';
import { EntityNodeImpl, EpisodicNodeImpl } from '../core/nodes';
import { EntityEdgeImpl } from '../core/edges';
describe('Graphzep Core', () => {
    const mockDriver = {
        provider: GraphProvider.NEO4J,
        executeQuery: mock.fn(async (...args) => []),
        close: mock.fn(async () => { }),
    };
    const mockLLMClient = {
    ...
    const mockEmbedder = {
        ...
        embedBatch: mock.fn(async (texts) => texts.map(() => new Array(384).fill(0.1))),
    };
    let graphzep;
    before(() => {
    ...
    describe('search', () => {
        it('should search for nodes by similarity', async () => {
            ...
            mockDriver.executeQuery.mock.mockImplementation(async (query) => {
    ...
    describe('node operations', () => {
        it('should get node by UUID', async () => {
            ...
            mockDriver.executeQuery.mock.mockImplementation(async () => [{ n: nodeData }]);
        ...
        it('should delete node by UUID', async () => {
            ...
            const deleteCalls = mockDriver.executeQuery.mock.calls.filter((call) => call.arguments[0].includes('DELETE'));
