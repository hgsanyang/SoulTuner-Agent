...
export class ShoeBotAgent {
  ...
  async initialize(clearData: boolean = false): Promise<void> {
    ...
    if (clearData) {
      ...
      // Note: clearData would be implemented at the driver level
    }
  ...
  private async ingestProductsData(): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < products.length; i++) {
        ...
        if (i % 10 === 0) {
          logger.log(`Loaded ${i + 1}/${products.length} products...`);
        }
      }
      
      logger.log(`Successfully loaded ${products.length} products`);
...
${shoeData ? `Available shoe information:\n${shoeData}` : ''}`;
    ...
    // Add the interaction to Graphzep graph asynchronously
    this.addInteractionToGraph(userName, userInput, response);
