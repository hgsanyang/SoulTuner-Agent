#!/usr/bin/env python
"""Build a manifest of the local NetEase cache and compare it with the catalog.

This command reads file names and sizes to make a cheap inventory. Decoding and
staging are handled by ``scripts/import_netease_cache_audio.py`` so inventory
reports remain fast and repeatable. Cache names carry the source id and quality:

    {song_id}-{quality}-{md5}.uc      e.g. 1054603-320-1964b0….uc

The manifest answers which tracks are present, which are already in the library,
and which need metadata or duplicate review.
    ...
    async with aiohttp.ClientSession() as session:
        for start in range(0, len(song_ids), batch):
            ...
            for track in payload.get("songs") or []:
                ...
                publish_time = int(album.get("publishTime") or track.get("publishTime") or 0) if isinstance(album, dict) else 0
                release_year = 0
                if publish_time:
                    from datetime import datetime

                    try:
                        release_year = datetime.fromtimestamp(publish_time / 1000).year
                ...
                found[str(track.get("id"))] = {
                    ...
                    "aliases": [
                        str(value).strip()
                        for value in [*(track.get("alia") or []), *(track.get("tns") or [])]
                        if str(value).strip()
                    ],
                }
    return found
...
def classify(
    ...
    for entry in entries:
        ...
        if meta:
            ...
            entry.release_year = int(meta.get("release_year") or 0)
            entry.album_id = str(meta.get("album_id") or "")
            entry.cover_url = str(meta.get("cover_url") or "")
            entry.aliases = list(meta.get("aliases") or [])
...
def main() -> int:
    ...
    print("\n清单已生成；需要验证音频与入库时运行 import_netease_cache_audio.py。")
    return 0
