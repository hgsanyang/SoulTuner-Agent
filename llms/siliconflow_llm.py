...
⚠️ DEPRECATED: 此模块已被 llms/multi_llm.py 完全替代。
请使用 `from llms.multi_llm import get_chat_model` 代替。
保留此文件仅为向后兼容，后续版本将删除。
"""

import warnings
warnings.warn(
    "siliconflow_llm 模块已废弃，请使用 llms.multi_llm.get_chat_model('siliconflow') 代替",
    DeprecationWarning,
    stacklevel=2
)
...
class SiliconFlowLLM(BaseLLM):
    ...
    def get_default_model(self) -> str:
        ...
        try:
            from config.settings_loader import load_settings_from_json
            settings = load_settings_from_json()
            return settings.get("SILICONFLOW_CHAT_MODEL", "deepseek-ai/DeepSeek-V3.2")
        except:
            # 如果都失败，使用默认值
