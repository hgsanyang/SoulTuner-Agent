...
1. **graph_search** — 含实体名（歌手/歌名）或纯硬属性标签（流派/语言/地区）的简单组合，不含场景词或描述性语言。注意：有明确实体+情绪词时也可走 graph_search（实体精确匹配+情绪标签够用）
   - 例："周杰伦的情歌" "英文摇滚" "日语歌" "来点摇滚"
2. **hybrid_search** — 含场景词/主观体验/画面感/声学描述中的任意一种，**或者**无实体但同时有情绪词+流派/语言标签
   - 例："上班路上推荐歌" "开车的歌" "运动时听的摇滚" "深情的国摇" "来点悲伤的金属" "想听温柔的民谣"
3. **vector_search** — 纯情绪/氛围/画面感，无任何可匹配的实体或硬属性标签
   - 例："我心情很差" "温暖治愈的感觉"
...
- 输入只包含歌手名/歌曲名 和/或 流派/语言/地区等纯属性标签，不含场景词或描述性语言
- 有明确实体（歌手/歌名）+ 情绪词时，可以保持 graph_search（实体精确匹配+情绪标签足够）
- ✅ "周杰伦的歌" → graph_search（只有实体，无场景无情绪）
- ✅ "英文摇滚" → graph_search（只有语言+流派标签）
- ✅ "日语歌" → graph_search（只有语言标签）
- ✅ "周杰伦的情歌" → graph_search（有明确歌手实体+情绪标签，graph足够精确）

**规则 2.5（新增）：无实体 + 情绪词 + 流派/语言/地区标签 → hybrid_search**
- 当没有明确歌手/歌名实体，但同时出现情绪词（深情/悲伤/热血/温柔/治愈/浪漫等）和流派/语言/地区标签时，走 hybrid_search
- 理由：graph 引擎用流派/语言标签粗筛候选集，vector 引擎用情绪的声学特征精排
- 关键信号词："想听" "来点" "推荐" "有没有" 等隐含主观需求的表达也计入
- ✅ "深情的国摇" → hybrid_search（无实体+情绪'深情'+流派'摇滚'+语言'Chinese'）
- ✅ "来点悲伤的金属" → hybrid_search（无实体+情绪'悲伤'+流派'金属'）
- ✅ "想听温柔的民谣" → hybrid_search（无实体+情绪'温柔'+流派'民谣'）
- ❌ "开心的歌" → vector_search（有情绪但无流派/语言标签，graph发挥不了作用）

**规则 3：有标签词 + 主观描述/体验/画面感 → hybrid_search**
- 用户输入中出现描述性语言（形容词、画面、主观感受、身体体验等）
- 关键信号词："感觉" "那种" "让我" "沸腾" "砸" "冲击" "飘" "空旷" "飞驰" "想听" "来点" "推荐" "有没有" 等体验性表达
...
用户: "有没有深情些的国摇，想听"
{{"intent_type": "hybrid_search", "parameters": {{"query": "深情 国摇", "entities": []}}, "context": "深情风格的国产摇滚推荐", "retrieval_plan": {{"use_graph": true, "graph_entities": [], "graph_genre_filter": "rock", "graph_scenario_filter": null, "graph_mood_filter": "深情", "graph_language_filter": "Chinese", "graph_region_filter": null, "use_vector": true, "vector_acoustic_query": "Emotional Chinese rock with sentimental vocals and passionate melody. Mid-tempo 80-110 BPM with warm distorted electric guitars, expressive singing, intimate atmospheric production. Deep romantic feeling with rock energy.", "use_web_search": false, "web_search_keywords": ""}}, "reasoning": "规则2.5：无实体+情绪词'深情'+流派'摇滚'+语言'Chinese'，hybrid_search"}}

用户: "来点悲伤的民谣"
{{"intent_type": "hybrid_search", "parameters": {{"query": "悲伤 民谣", "entities": []}}, "context": "悲伤氛围的民谣推荐", "retrieval_plan": {{"use_graph": true, "graph_entities": [], "graph_genre_filter": "folk", "graph_scenario_filter": null, "graph_mood_filter": "悲伤", "graph_language_filter": null, "graph_region_filter": null, "use_vector": true, "vector_acoustic_query": "Sad melancholic folk music with acoustic guitar fingerpicking. Slow tempo 60-85 BPM, minor key, intimate and vulnerable vocals. Warm gentle production with sorrowful emotional depth.", "use_web_search": false, "web_search_keywords": ""}}, "reasoning": "规则2.5：无实体+情绪词'悲伤'+流派'民谣'，hybrid_search"}}
