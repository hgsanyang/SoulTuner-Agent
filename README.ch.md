...
- 🌐 **本地曲库不够？证据驱动联网发现** — 与本地召回并行的补充路线用 planner 原生联网搜索找榜单/口碑/歌单支撑的歌；本地缺歌或缺年份等元数据时另有 Catalog Gap 兜底
...
>
> 融合知识图谱（Neo4j）、MuQ-MuLan 文搜音主锚、M2D-CLAP / OMAR-RQ 辅助音频表示、大语言模型和 MemoryGateway 记忆层，通过 LangGraph 编排的多节点 Agent 工作流，实现多路混合检索、加权 RRF 融合、Neo4j 图距离加权、SSE 流式推荐、证据驱动联网发现、音乐旅程编排和用户行为数据飞轮。
...
| 特性                       | 说明                                                                 |
| -------------------------- | -------------------------------------------------------------------- |
| 🔀**Hybrid RAG**     | 图谱 / 稠密双路内容召回，加权 RRF 融合；个性化与冷门探索作为召回后加分/减分项 |
| 🎵**多模态文搜音** | MuQ-MuLan 负责中文优先的文搜音召回，M2D-CLAP 自动回退，OMAR-RQ 补充声学相似性 |
| 🧠**长期记忆**       | MemoryGateway 统一入口：本地 append-only L0/L1/L2 账本（证据绑定、TTL、LLM 自命名场景作用域）+ Neo4j 行为热路径；BGE 语义相关性、不可用时 fail-closed；每条偏好可查看/编辑/删除 |
| 📊**粗排+探索**      | Graph Affinity 粗排截断 + Thompson Sampling 冷门探索槽               |
| 🤖**智能意图识别**   | 分层意图表示 `hard_constraints / soft_intent / hints` + 多轮继承      |
| 👤**用户画像**       | 前端可视化画像面板，流派/情绪/场景/语言偏好 → Neo4j 热路径 + 本地记忆账本 |
| 🌐**联网发现**       | 开关（默认开，关闭即纯本地）；与本地召回并行的证据驱动补充路线——planner 原生联网搜索找榜单/口碑/歌单支撑的歌，去重 + 可播放校验；本地缺口时另有 Catalog Gap 兜底 |
...
│                                                                     │
│  GraphRAG · Dense KNN · 证据驱动联网路线 · Catalog Gap 兜底         │
│         └──────────────────┬───────────────────┘                   │
│                            ▼                                        │
...
| **长期记忆**   | MemoryGateway（本地 append-only L0/L1/L2 SQLite 账本 + Neo4j 热路径 + BGE 相关性）；GraphZep/Mem0 为可选、非默认、隔离的 legacy 适配器 |
| **联网发现**   | 证据驱动并行补充路线（planner 原生联网搜索）+ Catalog Gap 兜底 + 可播放解析                |
...
用户查询 → Planner (LLM) 输出分层计划
   ...
   ┌──────────┬──────────┐
   ▼          ▼
GraphRAG   Dense KNN                  ← Step 1: 双路内容召回
(Neo4j)   (MuQ+OMAR)
   └──────────┴──────────┘
              ▼
...
双路召回会保留来源与排名，最终推荐卡片可展示“图谱检索 / 向量检索 / 联网候选”等来源信息，方便用户理解结果从哪里来。
...
MemoryGateway 统一处理用户画像、行为反馈和长期记忆。默认走本地 append-only L0/L1/L2 账本（原始事件 → 显式偏好 → 证据绑定的推断偏好，带 TTL、场景作用域、可编辑可删除）+ Neo4j 结构化行为热路径，相关性由本地 BGE 语义模型判定、不可用时 fail-closed；GraphZep/Mem0 仅作为可选、非默认、隔离的 legacy 适配器，不进入默认链路。
...
│   ├── web_search_aggregator.py # 可选联网发现适配器
...
│
├── graphzep_service/           # legacy GraphZep 适配器（可选、非默认）
