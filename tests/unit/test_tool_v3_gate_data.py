...
def test_review_outputs_keep_false_clarifications_out_of_trainable_data():
    reviews = _read_jsonl(
        ROOT / "data/sft/reviews/v3_ambiguous_review.jsonl"
    )
    trainable = _read_jsonl(
        ROOT / "data/sft/reviews/v3_resolved_trainable.jsonl"
    )
    excluded = {
        row["episode_id"]
        for row in reviews
