...
8. 判断歌曲的音乐流派 (genre)，提取 1~2 个最准确的流派标签。
   必须使用以下固定的英文值，不要发明新词：
   "Rock", "Pop", "Folk", "Indie", "Alternative", "Electronic", "Hip-Hop", "R&B", "Jazz", "Classical",
   "Metal", "Punk", "Country", "Blues", "Soul", "Ballad", "Ambient", "Post-Rock", "Shoegaze",
   "Singer-Songwriter", "Acoustic", "Experimental", "World Music", "Mandopop", "Cantopop", "J-Pop", "K-Pop"
   - 例如：陈粒 → "Folk", "Indie"；草东没有派对 → "Alternative", "Rock"；窦唯 → "Rock", "Ambient"
   - 如果是纯器乐/古典，可选 "Classical" 或 "Ambient"
9. 所有标签使用英文单词，保持专业和统一。不要发明已有近义词之外的新标签。

【联网搜索要求】你拥有联网搜索能力，这是你完成本任务的主要工具之一。对于每一首歌，请：
- 搜索确认歌手的 **来源地区**（尤其是独立乐队、小众艺术家、日本/台湾/香港乐手等）
- 搜索确认 **音乐流派**（尤其是风格融合、看名字无法判断流派的歌手）
- 有歌词较少的歌，搜索该歌手/专辑的风格背景来辅助推断情绪和场景标签
- 对艺名/译名歌手（如台湾、香港、日本、独立乐队）搜索真实身份信息
...
def generate_prompts(force_all: bool = False):
    ...
    if os.path.exists(result_json_path) and not force_all:
        try:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="生成 Gemini 歌曲标签分析提示词")
    parser.add_argument(
        "--force-all", action="store_true",
        help="忽略 gemini_result.json 中的已处理记录，为所有 1000+ 首歌重新生成提示词（用于全库重新标注）"
    )
    args = parser.parse_args()
    generate_prompts(force_all=args.force_all)
