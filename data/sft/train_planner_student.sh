...
#   MODEL=Qwen/Qwen3.6-9B bash train_planner_student.sh
...
python - "$TRAIN_FILE" "$VAL_FILE" "$MODEL" "$MODEL_TAG" "$PREFLIGHT_DIR" "$RUN_RECORD" \
...
python - "$RUN_RECORD" "$TRAIN_FILE" "$VAL_FILE" "$MANIFEST_FILE" "$MODEL" <<'PY'
import hashlib
import json
import sys

record_path, train_file, val_file, manifest_file, model = sys.argv[1:6]
