...
PREFLIGHT_RUN_ID="${PREFLIGHT_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"
OUTPUT_DIR="${OUTPUT_DIR:-./output/planner-student-${MODEL_TAG}-lora}"
...
python - "$TRAIN_FILE" "$VAL_FILE" "$MODEL" "$MODEL_TAG" "$PREFLIGHT_DIR" "$PREFLIGHT_DIR/environment.json" <<'PY'
import hashlib
import json
import os
import platform
...
train_file, val_file, model, model_tag, preflight_dir, output_file = sys.argv[1:7]
...
def step_number(record):
    direct = record.get("global_step") or record.get("current_steps")
    if direct is not None:
        return int(direct)
    # ms-swift 4.4 writes values such as "12/50" under this combined key.
    combined = str(record.get("global_step/max_steps") or "")
    try:
        return int(combined.split("/", 1)[0])
...
python - "$PREFLIGHT_DIR/environment.json" "$TRAIN_FILE" "$VAL_FILE" <<'PY'
import hashlib
import json
import sys

manifest_path, train_file, val_file = sys.argv[1:4]
...
if grep -q '<think>' "$OUTPUT_DIR/eval_predictions.jsonl"; then
  echo "FAIL: 推理输出仍包含 <think>，不能把环境配置问题误判成 schema 失败"
  exit 8
fi
