...
class MusicKnowledgeStore:
    ...
    def deprecate_source_provider(
        self,
        provider: str,
        *,
        replacement_provider: str = "legacy_deprecated",
        max_confidence: float = 0.3,
    ) -> dict[str, int]:
        """Lower trust for legacy source providers without deleting audit data."""

        self.initialize()
        provider = str(provider or "").strip()
        if not provider:
            return {"sources": 0, "song_cards": 0, "artist_cards": 0}
        replacement = str(replacement_provider or "legacy_deprecated").strip()
        confidence = clamp_confidence(max_confidence, default=0.3)
        with self.connect() as conn:
            source_rows = conn.execute(
                "SELECT id FROM sources WHERE provider = ?",
                (provider,),
            ).fetchall()
            ...
            song_count = int(
                conn.execute(
                    f"SELECT count(*) AS n FROM song_cards WHERE source_id IN ({placeholders})",
                    source_ids,
                ).fetchone()["n"]
                or 0
            )
            artist_count = int(
                conn.execute(
                    f"SELECT count(*) AS n FROM artist_cards WHERE source_id IN ({placeholders})",
                    source_ids,
                ).fetchone()["n"]
                or 0
            )
            conn.execute(
                ...
                SET confidence = min(confidence, ?),
                    details_json = json_set(
                        ...
                        '$.deprecated_source',
                        json_object('provider', ?, 'reason', 'legacy aggregator answer without traceable page evidence')
                    ),
                    updated_at = ?
                WHERE source_id IN ({placeholders})
                """,
                [confidence, provider, _now_ms(), *source_ids],
            )
            conn.execute(
                f"""
                UPDATE artist_cards
            ...
            conn.execute(
                ...
                """,
                [replacement, confidence, _now_ms(), *source_ids],
            )
            return {"sources": len(source_ids), "song_cards": song_count, "artist_cards": artist_count}
