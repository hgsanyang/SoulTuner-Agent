...
_DECODE_CHUNK_BYTES: Final = 1 << 20
_UC_XOR_TABLE: Final = bytes(byte ^ 0xA3 for byte in range(256))
...
class BaseAudioDecoder(ABC):
    """Strategy interface: turn one supported input into usable audio bytes.
    ...
    def decode(self, input_path: str, output_path: str) -> bool:
        """Write decoded or converted audio to ``output_path``.
...
class ProtectedContainerDecoder(BaseAudioDecoder):
    """Recognise known containers that have no decoder registered yet."""
    ...
    def decode(self, input_path: str, output_path: str) -> bool:
        ...
        raise ProtectedContainerError(
            f"{src.name}: no decoder is registered for "
            f"'{src.suffix.lower() or 'unknown'}'"
        )
...
class UcCacheDecoder(BaseAudioDecoder):
    ...
    def decode(self, input_path: str, output_path: str) -> bool:
        src, dst = _validated_paths(input_path, output_path)
        temporary = dst.with_name(f".{dst.name}.{uuid.uuid4().hex}.part")
        try:
            total = 0
            with src.open("rb") as source, temporary.open("xb") as target:
                while chunk := source.read(_DECODE_CHUNK_BYTES):
                    decoded = chunk.translate(_UC_XOR_TABLE)
                    target.write(decoded)
                    total += len(decoded)
            if total == 0:
                raise AudioDecodeError(f"{src.name}: decoded payload is empty")
            ...
            dst.unlink(missing_ok=True)
            raise AudioDecodeError(
                f"failed to decode {src.name}: {type(exc).__name__}: {exc}"
            ) from exc
...
def process_audio(
    ...
    *,
    ...
    if isinstance(decoder, UcCacheDecoder):
        return _process_wrapped_audio(
            src,
            target_dir,
            decoder,
            overwrite=overwrite,
...
def _process_wrapped_audio(
    ...
    *,
    ...
    """Decode first, identify second, and only then choose the final filename."""
    from services.audio_format import detect_file, is_lossless, mime_for, probe, suffix_for

    decoded = target_dir / f".{src.stem}.{uuid.uuid4().hex}.decoded"
    converted: Path | None = None
    try:
        decoder.decode(str(src), str(decoded))
        container = detect_file(decoded)
        if not container:
            raise AudioDecodeError(
                f"{src.name}: decoded bytes are not a recognised audio container"
            )
        _verify_audio_payload(decoded, container, probe)

        requested = str(playback_format or "").strip().lower().lstrip(".")
        if requested and requested not in {"mp3"}:
            raise UnsupportedAudioFormatException(
                f"derived playback format '{requested}' is not supported"
            )

        final_container = requested or container
        final_suffix = suffix_for(final_container)
        if not final_suffix:
            raise UnsupportedAudioFormatException(
                f"no canonical suffix is registered for decoded container '{final_container}'"
            )
        dst = target_dir / f"{src.stem}{final_suffix}"
        if dst.exists() and not overwrite:
            existing_container = detect_file(dst)
            if existing_container != final_container:
                raise AudioDecodeError(
                    f"existing output {dst.name} contains '{existing_container or 'unknown'}', "
                    f"expected '{final_container}'"
                )
            ...
            return DecodeResult(
                ...
                mime_type=mime_for(existing_container),
                lossless=is_lossless(existing_container),
            )

        transcoded = bool(requested and requested != container)
        if transcoded:
            converted = target_dir / f".{src.stem}.{uuid.uuid4().hex}.converted{final_suffix}"
            FfmpegTranscodeDecoder().decode(str(decoded), str(converted))
            _verify_audio_payload(converted, final_container, probe)
            os.replace(converted, dst)
        else:
            os.replace(decoded, dst)
        ...
        return DecodeResult(
            ...
            mime_type=mime_for(final_container),
            lossless=is_lossless(final_container),
        )
    except Exception:
        decoded.unlink(missing_ok=True)
...
def _verify_audio_payload(
    path: Path,
    expected_container: str,
    probe_fn: Callable[[str | Path], dict[str, Any]],
) -> None:
    """Fail closed on truncated decoded payloads when ffprobe is available."""
    from services.audio_format import detect_file

    actual = detect_file(path)
    if actual != expected_container:
        raise AudioDecodeError(
            f"{path.name}: detected '{actual or 'unknown'}', expected '{expected_container}'"
        )
    if path.stat().st_size == 0:
        raise AudioDecodeError(f"{path.name}: audio payload is empty")
    if shutil.which("ffprobe") is None:
        logger.warning("[audio-decoder] ffprobe unavailable; only container signature was verified")
        return
    details = probe_fn(path)
    if not details or not details.get("codec") or not details.get("channels"):
        raise AudioDecodeError(f"{path.name}: ffprobe could not verify an audio stream")
