                    ...
                    >
                        ...
                        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
                            ...
                            <div style={{ padding: '1.5rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
                                <button onClick={() => setExpanded(false)} aria-label={t("收起全屏播放器")} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', padding: '0.25rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.2rem', backgroundColor: 'rgba(255,255,255,0.09)', borderRadius: '2rem', padding: '0.22rem' }}>
                                    {([['lyrics', t('歌词')], ['album', t('专辑')], ['artist', t('歌手')]] as const).map(([tab, label]) => (
                                        <button
                                            key={tab}
            ...
            <div style={{
                ...
                <div style={{ flex: '1', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                        <button onClick={() => setPlayMode(playMode === 'sequence' ? 'random' : playMode === 'random' ? 'loop' : 'sequence')} aria-label={t("切换播放模式")} style={{ background: 'none', border: 'none', color: theme.colors.text.muted, cursor: 'pointer' }} title={t("切换播放模式")}>
                            <PlayModeIcon />
                        </button>
                        <button onClick={playPrev} aria-label={t("上一首")} style={{ background: 'none', border: 'none', color: theme.colors.text.primary, cursor: 'pointer' }}>
                        ...
                        <button onClick={togglePlay} aria-label={isPlaying ? t('暂停') : t('播放')} style={{ width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: 'none' }}>
                            {isPlaying
                        ...
                        <button onClick={playNext} aria-label={t("下一首")} style={{ background: 'none', border: 'none', color: theme.colors.text.primary, cursor: 'pointer' }}>
                        ...
                        {/* 播放列表按钮（替换原下载按钮） */}
                        <button onClick={() => setShowQueue(p => !p)} title={t("播放列表")} aria-label={t("播放列表")} style={{ background: 'none', border: 'none', color: showQueue ? '#fff' : theme.colors.text.muted, cursor: 'pointer', transition: 'color 0.2s' }}>
                ...
                <div style={{ flex: '1', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '1rem' }}>
                    <button aria-label={t("音量")} style={{ background: 'none', border: 'none', color: theme.colors.text.muted, cursor: 'pointer' }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <button onClick={() => setExpanded(prev => !prev)} title={t("展开/收起播放器")} aria-label={isExpanded ? t('收起播放器') : t('展开播放器')} style={{ background: 'none', border: 'none', color: theme.colors.text.muted, cursor: 'pointer', marginLeft: '0.5rem' }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
