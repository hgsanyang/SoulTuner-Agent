...
(Neo4j)   (MuQ+OMAR) (标题/歌手/歌词)
  ...
  Step 5: 召回后加分/减分 + TS 探索     ← 个性化/新歌/冷门加分，过曝按时间衰减降分
              ▼
  Step 6: 内容双锚归一化精排            ← 辅助语义(M2D-CLAP) + 声学(OMAR-RQ)
              ▼
...
- **内容双锚精排**：辅助语义锚 `(cosine+1)/2`（M2D-CLAP）+ 声学锚 `(cosine+1)/2`（OMAR-RQ 质心）归一化融合；个性化只走限幅 `post_recall_adjustments`，不会以 25% 锚权重压过当前查询。
