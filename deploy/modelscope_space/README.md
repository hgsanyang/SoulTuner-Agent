...
4. Dense 默认以 MuQ 直接理解中文语义，再用 OMAR 做声学重排；M2D 仅保留给纯 CPU 档；
5. 结果融合、推荐理由与受控输出；
6. 喜欢、跳过和不喜欢反馈写入当前会话记忆；
7. 同一 35B 基座同时提供两个受控角色：SoulTuner LoRA 只负责 Planner，基座模型负责自然语言对话、推荐开场白和解释；
8. 统一输入框的每一轮消息都先进入 Planner；Python 不使用“推荐/来点”等触发词猜测用户意图，确定性 Guard 只校验结构、检索证据与联网副作用；
9. 当前播放歌曲、上轮结构化计划和可见对话历史共同进入下一轮上下文，推荐细化会刷新曲目，解释或闲聊则保留现有播放器状态。
10. 支持 1,806 首公开曲目的封面展示、检索、播放、反馈与会话记忆；GPU 部署的就绪门槛为 MuQ + OMAR，M2D 不再阻塞启动。
...
当前公开创空间的目标配置是 AMD MI308X 192 GB 与 ROCm 镜像；只有平台成功分配实例且
运行日志确认 ROCm/HIP 设备后，才能把某次运行认定为 AMD 部署。资源紧张导致的调度失败
发生在应用启动之前，不代表下面的应用启动流程已经执行。实例可用时，Gradio 会先绑定公开端口，
